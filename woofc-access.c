...
void WooFProcessPut(zmsg_t* req_msg, zsock_t* receiver) {
    ...
    if (element == NULL) { /* element too big */
        fprintf(stderr,
                "WooFMsgProcessPut: woof_name: %s, handler: %s, size %lu too big\n",
                woof_name,
                hand_name,
                copy_size);
        fflush(stderr);
...
unsigned long WooFMsgGetLatestSeqno(char* woof_name, char* cause_woof_name, unsigned long cause_woof_latest_seq_no) {
    ...
    /*
     * this is a GetLatestSeqno message
     */
    memset(buffer, 0, sizeof(buffer));
    sprintf(buffer, "%02lu", WOOF_MSG_GET_LATEST_SEQNO);
    frame = zframe_new(buffer, strlen(buffer));
    if (frame == NULL) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s no frame for WOOF_MSG_GET_LATEST_SEQNO "
...
#ifdef DEBUG
    printf("WooFMsgGetLatestSeqno: woof: %s got WOOF_MSG_GET_LATEST_SEQNO (%s) command "
           "frame frame\n",
           woof_name,
           buffer);
    fflush(stdout);
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s can't append WOOF_MSG_GET_LATEST_SEQNO "
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s can't append woof_name to frame to "
                "server at %s\n",
                woof_name,
                endpoint);
    ...
    if (err < 0) {
        fprintf(stderr,
                ...
                woof_name,
                endpoint);
        perror("WooFMsgGetLatestSeqno: couldn't append cause_host frame");
        zframe_destroy(&frame);
        zmsg_destroy(&msg);
        return (-1);
    }
#ifdef DEBUG
    printf("WooFMsgGetLatestSeqno: woof: %s added cause_host to frame\n", woof_name);
    fflush(stdout);
#endif
    ...
    if (err < 0) {
        fprintf(stderr,
                ...
                woof_name,
                endpoint);
        perror("WooFMsgGetLatestSeqno: couldn't append cause_seq_no frame");
        zframe_destroy(&frame);
        zmsg_destroy(&msg);
        return (-1);
    }
#ifdef DEBUG
    printf("WooFMsgGetLatestSeqno: woof: %s added cause_seq_no to frame\n", woof_name);
    fflush(stdout);
#endif

    /*
     * make a frame for the cause_woof_name
     */
    if (cause_woof_name == NULL) {
        frame = zframe_new_empty();
    } else {
        frame = zframe_new(cause_woof_name, strlen(cause_woof_name));
    }
    if (frame == NULL) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s no frame for cause_woof_name to server "
                "at %s\n",
                woof_name,
                endpoint);
        perror("WooFMsgGetLatestSeqno: couldn't get new frame");
...
#ifdef DEBUG
    printf("WooFMsgGetLatestSeqno: woof: %s got cause_woof_name frame\n", woof_name);
    fflush(stdout);
#endif
    /*
     * add the cause_woof_name frame to the msg
     */
    err = zmsg_append(msg, &frame);
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s can't append cause_woof_name to frame "
                "to server at %s\n",
                woof_name,
                endpoint);
        perror("WooFMsgGetLatestSeqno: couldn't append cause_woof_name frame");
        zframe_destroy(&frame);
        zmsg_destroy(&msg);
        return (-1);
    }
#ifdef DEBUG
    printf("WooFMsgGetLatestSeqno: woof: %s added cause_woof_name to frame\n", woof_name);
    fflush(stdout);
#endif
    ...
    if (frame == NULL) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s no frame for cause_woof_latest_seq_no "
                "to server at %s\n",
                woof_name,
                endpoint);
        perror("WooFMsgGetLatestSeqno: couldn't get new frame");
...
#ifdef DEBUG
    printf("WooFMsgGetLatestSeqno: woof: %s got frame for cause_woof_latest_seq_no %lu\n",
           woof_name,
           cause_woof_latest_seq_no);
    fflush(stdout);
#endif
    /*
     * add the cause_woof_latest_seq_no frame to the msg
     */
    err = zmsg_append(msg, &frame);
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s can't append cause_woof_latest_seq_no "
                "to frame to server at %s\n",
                woof_name,
                endpoint);
        perror("WooFMsgGetLatestSeqno: couldn't append cause_woof_latest_seq_no frame");
        zframe_destroy(&frame);
        zmsg_destroy(&msg);
        return (-1);
    }
    ...
    if (r_msg == NULL) {
        fprintf(stderr,
                "WooFMsgGetLatestSeqno: woof: %s couldn't recv msg for element size from "
                "server at %s\n",
                woof_name,
                endpoint);
...
#ifdef DEBUG
        printf("WooFMsgThread: processing msg with tag: %lu\n", tag);
        fflush(stdout);
#endif
...
unsigned long WooFMsgGetElSize(char* woof_name) {
     ...
     */
    memset(buffer, 0, sizeof(buffer));
    sprintf(buffer, "%02lu", WOOF_MSG_GET_EL_SIZE);
    frame = zframe_new(buffer, strlen(buffer));
    if (frame == NULL) {
        fprintf(stderr,
                "WooFMsgGetElSize: woof: %s no frame for WOOF_MSG_GET_EL_SIZE command in "
                "to server at %s\n",
                woof_name,
                endpoint);
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGetElSize: woof: %s can't append WOOF_MSG_GET_EL_SIZE command "
                "frame to msg for server at %s\n",
                woof_name,
                endpoint);
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGetElSize: woof: %s can't append woof_name to frame to server at "
                "%s\n",
                woof_name,
                endpoint);
...
unsigned long WooFMsgGetTail(char* woof_name, void* elements, unsigned long el_size, int el_count) {
     ...
     */
    memset(buffer, 0, sizeof(buffer));
    sprintf(buffer, "%02lu", WOOF_MSG_GET_TAIL);
    frame = zframe_new(buffer, strlen(buffer));
    if (frame == NULL) {
        fprintf(stderr,
                "WooFMsgGetTail: woof: %s no frame for WOOF_MSG_GET_TAIL command in to "
                "server at %s\n",
                woof_name,
                endpoint);
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGetTAIL: woof: %s can't append WOOF_MSG_GET_TAIL command frame "
    ...
    if (r_msg == NULL) {
        fprintf(stderr,
                "WooFMsgGetTail: woof: %s couldn't recv msg for element size from server "
                "at %s\n",
                woof_name,
                endpoint);
        perror("WooFMsgGetElSize: no response received");
...
unsigned long WooFMsgPut(char* woof_name, char* hand_name, void* element, unsigned long el_size) {
     ...
     */
    memset(buffer, 0, sizeof(buffer));
    sprintf(buffer, "%02lu", WOOF_MSG_PUT);
    frame = zframe_new(buffer, strlen(buffer));
    if (frame == NULL) {
        fprintf(
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgPut: woof: %s can't append WOOF_MSG_PUT command frame to msg for "
                "server at %s\n",
                woof_name,
                endpoint);
    ...
    if (frame == NULL) {
        fprintf(stderr,
                "WooFMsgPut: woof: %s no frame for handler name %s to server at %s\n",
                woof_name,
                hand_name,
                endpoint);
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgPut: woof: %s couldn't append frame for handler name %s to "
                "server at %s\n",
                woof_name,
                hand_name,
                endpoint);
    ...
    if (frame == NULL) {
        fprintf(stderr,
                "WooFMsgPut: woof: %s no frame size %lu for handler name %s to server at "
                "%s\n",
                woof_name,
                el_size,
                hand_name,
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgPut: woof: %s couldn't append frame for element size %lu name %s "
                "to server at %s\n",
                woof_name,
                el_size,
                hand_name,
    ...
    if (r_msg == NULL) {
        fprintf(stderr,
                "WooFMsgPut: woof: %s couldn't recv msg for element size %lu name %s to "
                "server at %s\n",
                woof_name,
                el_size,
                hand_name,
        ...
        if (r_frame == NULL) {
            fprintf(stderr,
                    "WooFMsgPut: woof: %s no recv frame for element size %lu name %s to "
                    "server at %s\n",
                    woof_name,
                    el_size,
                    hand_name,
...
int WooFMsgGet(char* woof_name, void* element, unsigned long el_size, unsigned long seq_no) {
    ...
    if (err < 0) {
        fprintf(stderr,
                "WooFMsgGet: woof: %s can't append WOOF_MSG_GET command frame to msg for "
                "server at %s\n",
                woof_name,
                endpoint);
    ...
    if (r_msg == NULL) {
        fprintf(stderr,
                "WooFMsgGet: woof: %s couldn't recv msg for seq_no %lu name %s to server "
                "at %s\n",
                woof_name,
                seq_no,
                endpoint);
        ...
        if (r_frame == NULL) {
            fprintf(stderr,
                    "WooFMsgGet: woof: %s no recv frame for seq_no %lu to server at %s\n",
                    woof_name,
                    seq_no,
                    endpoint);
...
#ifdef DEBUG
            printf("WooFMsgGet: woof: %s copied %lu bytes for seq_no %lu message to "
                   "server at %s\n",
                   woof_name,
                   r_size,
                   seq_no,
