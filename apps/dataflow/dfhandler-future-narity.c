...
printf("DEBUG: SCAN 1 EXIT FOUND CLAIM(1)/PARTIAL(2): %d, prog: %s, node_no: %d ip_val: %f ip_port %d\n",
	node.state, prog, node.node_no, claim_node.ip_value, claim_node.ip_port);
#endif  
			return(0);
		}
		 ...
		 */
#ifdef DEBUG
printf("DEBUG: SCAN 1 ALL INPUTS RECIEVED: prog: %s, node_no: %d port_no: %d value: %f\n",
	prog, claim_node.node_no, claim_node.ip_port, claim_node.ip_value);
#endif 		 
		if((node.node_no == claim_node.node_no) &&
		   (node.state == WAITING) && 
