...
\end{figure}
\end{document}
In the figure, inputs tor the program (each defined by a \verb+dfaddoperand+
...
occuring before the other.  The first one (the one with a smaller sequence
...
partial fire and the original \verb+CLAIM+ then a second handler fired later
and exited.  This handler, then fires the node by calling \verb+dfOperation()+
to compute the result, and \verb+dfFireNode()+ as desribed above.  If a second
