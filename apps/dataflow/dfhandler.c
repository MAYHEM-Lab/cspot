...
int dfFireNode(WOOF* wf, char* prog, DFNODE* node, double result) {
    ...
    if (WooFInvalid(d_seqno)) {
        fprintf(stderr, "ERROR -- dfhandler could not retire in %s\n", prog);
        return (1);
    }
#ifdef DEBUG
     ...
     */
    if (node->dst_no == 0) {
        d_seqno = WooFPut(WooFGetFileName(wf), NULL, &result_operand);
#ifdef DEBUG
        printf("DEBUG: FINAL RESULT: %f\n", result);
#endif
    } else {
        d_seqno = WooFPut(WooFGetFileName(wf), "dfhandler", &result_operand);
    }
    if (WooFInvalid(d_seqno)) {
        fprintf(stderr, "ERROR -- dfhandler could not forward result to %s\n", WooFGetFileName(wf));
        return (1);
    }
#ifdef DEBUG
...
int dfhandler(WOOF* wf, unsigned long wf_seq_no, void* ptr) {
    ...
    if (WooFInvalid(c_seqno)) {
        fprintf(stderr, "ERROR -- dfhandler: claim seqno failed from %s\n", prog);
        return (1);
    }
    ...
    while (!WooFInvalid(s)) {
        ...
        printf("seqno=%li\n", s);
        printf("id=%d (op=%i) (dst=%i:%i), (inp=%i, val=%f), ",
               node.node_no,
               node.opcode,
               node.dst_no,
               node.dst_port,
    ...
    while (!WooFInvalid(seqno)) {
        ...
        if (err < 0) {
            fprintf(stderr, "ERROR -- dfhandler: get of %s failed at %lu\n", prog, seqno);
            return (1);
        }
        /*
         ...
         */
        if ((node.node_no == claim_node.node_no) && (node.state == CLAIM || node.state == PARTIAL) &&
            (seqno < c_seqno)) {
#ifdef DEBUG
            printf("DEBUG: SCAN 1 EXIT FOUND CLAIM(1)/PARTIAL(2): %d, prog: %s, node_no: %d ip_val: %f ip_port %d "
                   "seq_no %ld\n",
                   node.state,
                   prog,
                   node.node_no,
...
#ifdef DEBUG
        printf("DEBUG : claim_node_no : %d, claim_node_port : %d claim_node_state %d curr_node_no: %d curr_node_state: "
         ...
         */
        if ((node.node_no == claim_node.node_no) && (node.state == WAITING) && (seqno < c_seqno)) {
            ...
            if (node.recvd_val_ct == node.total_val_ct) {
                result = DFOperation(node.opcode, node.values, node.total_val_ct);
#ifdef DEBUG
