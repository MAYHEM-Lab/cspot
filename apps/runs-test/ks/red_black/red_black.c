...
RB *RBAddTree(RB *parent, double key, Hval value, RB *top)
{
	RB *node;
...
RB *RBFindNode(RB *node, double key)
{
	if(node == NULL)
	{
		return(NULL);
...
//		RBPrintTree(tree);
	...
	while(count > 10)
	{
		...
		{
			if(tree->prev->key > tree->prev->next->key)
			{
				printf("bad sequence\n");
				exit(1);
			}
