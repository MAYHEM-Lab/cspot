...
#define ARGS "i:"
char* Usage = "init_room_topic -i client_ip\n";
...
int main(int argc, char** argv) {
    ...
    if (dht_create_topic(ROOM_TEMP_TOPIC, sizeof(TEMP_EL), AVG_TEMP_HISTORY_LENGTH) < 0) {
        fprintf(stderr, "failed to create topic woof: %s\n", dht_error_msg);
        exit(1);
    }

    if (dht_register_topic(ROOM_TEMP_TOPIC, 5000) < 0) {
