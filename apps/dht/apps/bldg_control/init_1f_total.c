...
int main(int argc, char** argv) {
    ...
    if (dht_register_topic(TOPIC_1F_TOTAL, client_ip) < 0) {
    ...
    if (dht_subscribe(TOPIC_ROOM_101_TRAFFIC, client_ip, "h_floor_total") < 0) {
        fprintf(stderr, "failed to subscribe to topic: %s\n", dht_error_msg);
        exit(1);
    }
    if (dht_subscribe(TOPIC_ROOM_102_TRAFFIC, client_ip, "h_floor_total") < 0) {
        fprintf(stderr, "failed to subscribe to topic: %s\n", dht_error_msg);
        exit(1);
    }
    if (dht_subscribe(TOPIC_ROOM_103_TRAFFIC, client_ip, "h_floor_total") < 0) {
        fprintf(stderr, "failed to subscribe to topic: %s\n", dht_error_msg);
        exit(1);
    }
