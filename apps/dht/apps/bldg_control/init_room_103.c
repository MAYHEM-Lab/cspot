...
#define ARGS "i:"
char* Usage = "init_room_103 -i client_ip\n";
...
int main(int argc, char** argv) {
    ...
    if (dht_create_topic(TOPIC_ROOM_103_TRAFFIC, sizeof(TRAFFIC_EL), BLDG_HISTORY_LENGTH) < 0) {
        fprintf(stderr, "failed to create topic woof: %s\n", dht_error_msg);
        exit(1);
    }

    if (dht_register_topic(TOPIC_ROOM_103_TRAFFIC, client_ip) < 0) {
