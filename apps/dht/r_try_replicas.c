...
int r_try_replicas(WOOF* wf, unsigned long seq_no, void* ptr) {
    ...
    while (get_milliseconds() - begin < TRY_SUCCESSOR_REPLICAS_TIMEOUT) {
        ...
        unsigned long latest_node = checkedWooFGetLatestSeq(&blocked_nodes, node.addr, replica_woof);
        ...
        if (checkedWooFGet(&blocked_nodes, node.addr, replica_woof, &replica_node, latest_node) < 0) {
