...
int r_try_replicas(WOOF *wf, unsigned long seq_no, void *ptr) {
	...
	switch (arg->type) {
		case DHT_TRY_PREDECESSOR: {
			...
			int i;
			for (i = start_id + 1; i != start_id; i = (i + 1) % DHT_REPLICA_NUMBER) {
		...
		case DHT_TRY_FINGER: {
			...
			for (i = start_id + 1; i != start_id; i = (i + 1) % DHT_REPLICA_NUMBER) {
				...
				if (WooFInvalid(seq)) {
					log_warn("finger replica[%d] not responding: %s", i, finger_addr(&finger));
					continue;
				}
	...
	if (strcmp(arg->handler_name, "h_find_successor") == 0) {
