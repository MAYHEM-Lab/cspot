...
int h_timeout_checker(WOOF *wf, unsigned long seq_no, void *ptr) {
	...
	if (WooFInvalid(seq)) {
		log_error("couldn't increment the server's term to %lu and initialize an election");
		exit(1);
	}
	log_debug("incremented the current term to %lu and become a candidate", server_state.current_term);
