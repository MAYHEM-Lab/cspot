...
void *request_vote(void *arg) {
	...
	if (WooFInvalid(seq)) {
		log_warn("couldn't request vote from %s", thread_arg->member_woof);
	}
	free(arg);
}
...
int term_chair(WOOF *wf, unsigned long seq_no, void *ptr) {
	...
	if (WooFInvalid(last_term_entry)) {
		log_error("couldn't get the latest seqno from %s", RAFT_TERM_ENTRIES_WOOF);
		exit(1);
	}
	// log_debug("reviewing new term request from %lu to %lu", function_loop->last_reviewed_term_chair + 1, last_term_entry);
	if (function_loop->last_reviewed_term_chair >= last_term_entry) {
		...
		if (server_state.role == RAFT_LEADER) {
			...
			if (WooFInvalid(seq)) {
				log_error("couldn't queue the next function_loop: replicate_entries");
				exit(1);
			}
		} else {
	...
	for (i = function_loop->last_reviewed_term_chair + 1; i <= last_term_entry; ++i) {
		...
		if (err < 0) {
			log_error("couldn't get the term entry at %lu", i);
			exit(1);
		}
	...
	if (next_term > server_state.current_term || (next_term == server_state.current_term && next_role != server_state.role)) {
		...
		switch (next_role) {
			case RAFT_LEADER: {
				log_info("incremented the current term to %lu, role: LEADER", next_term);
				break;
			}
			case RAFT_FOLLOWER: {
				log_info("incremented the current term to %lu, role: FOLLOWER", next_term);
				break;
			}
			case RAFT_CANDIDATE: {
		...
		if (next_role != RAFT_LEADER) { // leader won't timeout
			...
			if (WooFInvalid(seq)) {
				log_error("couldn't put a new heartbeat for new term %lu", next_term);
				exit(1);
			}
			// log_debug("put a new heartbeat at term %lu", next_term);
		}
		...
		if (next_role == RAFT_CANDIDATE) {
			...
			if (WooFInvalid(vote_pool_seqno)) {
				log_error("couldn't get the latest seqno from %s", RAFT_REQUEST_VOTE_RESULT_WOOF);
				exit(1);
			}
