...
//             sprintf(raft_error_msg, "timeout after %d ms waiting for client_put result %lu\n", timeout, seq);
//             return RAFT_TIMEOUT;
//         }
...
//         if (timeout > 0 && get_milliseconds() - begin > timeout) {
//             sprintf(raft_error_msg, "timeout after %d ms waiting for commit_index %lu\n", timeout, result.index);
//             return RAFT_TIMEOUT;
//         }
...
//     uint64_t index, term;
//     if (raft_client_put_result(raft_leader, &index, &term, client_put_seqno) < 0) {
//         sprintf(raft_error_msg, "can't get index and term from client_put request");
//         return RAFT_ERROR;
//     }

//     uint64_t committed_term = raft_sync_get(raft_leader, data, index, 1);
