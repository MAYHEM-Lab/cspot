...
int check_append_result(RAFT_SERVER_STATE* server_state, RAFT_CHECK_APPEND_RESULT_ARG* arg) {
    ...
    if (arg->last_seen_result_seqno + 1 <= last_append_result_seqno) {
        log_debug("checking result from %lu to %lu", arg->last_seen_result_seqno + 1, last_append_result_seqno);
    }
