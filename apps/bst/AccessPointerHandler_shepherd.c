...
extern int AccessPointerHandler(WOOF *wf, unsigned long seq_no, void *farg);
...
int main(int argc, char **argv, char **envp)
{
	...
	while (1)
	{
		...
		fflush(stdout);
#endif
		err = AccessPointerHandler(wf, seq_no, (void *)farg);
#ifdef DEBUG
		...
		if (err == 0)
		{ //EOF means container doesn't need this process
			break;
		}
		else if (err < 0)
		{
