...
#define ACCESS_TIMING_ENABLED 0
...
void BST_insert(DI di){
    ...
    if(strncmp(parent_data.di.key, di.key, S3_KEY_SIZE) > 0){
...
void dump_data_woof(){
    ...
    for(i = 1; i <= WooFGetLatestSeqno(DATA_WOOF_NAME); ++i){
        ...
        fprintf(stdout, "%lu: %s %s %s\n", i, data.di.key, data.lw_name, data.pw_name);
        fflush(stdout);
    }
}
