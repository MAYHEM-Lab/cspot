...
while ( test $CNT -lt $BATCHCOUNT ) ; do
	...
	MIN=`echo $SUM4 -v min=$MIN '{if($1 < min){print $1}else{print min}}'`
	MIN=`echo $SUM4 -v min=$MIN '{if($2 < min){print $1}else{print min}}'`
	MIN=`echo $SUM4 -v min=$MIN '{if($3 < min){print $1}else{print min}}'`
	MIN=`echo $SUM4 -v min=$MIN '{if($4 < min){print $1}else{print min}}'`
...
BEST=`echo $PAYLOAD $MIN | awk '{print ($1/(1024*1024)) / ($2 / 1000.0)}'`
