...
template<class CapT, class CryptoModelT, class ReqParser, class Satisfies>
auto req_deserializer(typename CryptoModelT::signer_type& signer,
                      ReqParser&& parse,
                      Satisfies&& satisfies) {
    return [&signer,
            parse = std::forward<ReqParser>(parse),
            satisfies = std::forward<Satisfies>(satisfies),
            last_seq = uint64_t(0)](tos::span<const char> req) mutable
           -> tos::expected<decltype(parse(std::declval<tos::span<const uint8_t>>())),
                            deser_errors> {
        ...
        tos::imemory_stream cap_str(
            {(const char*)uc.item.as.bin.start, (size_t)uc.item.as.bin.length});

        auto cps = caps::deserialize<CapT, CryptoModelT>(cap_str);
