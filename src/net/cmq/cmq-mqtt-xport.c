...
//printf("pw: %s\n",MQTT_Proxy.pw);
    ...
    if(found == 5) {
	    ...
	    fprintf(stderr,
		"cmq_mqtt_proxy_init: %s contains incomplete mqtt proxy specification\n",credfile);
	    fprintf(stderr,
		"please ensure that %s contains proxy, namespace, broker_ip, user, and pw fields\n",credfile);
	    fprintf(stderr,"mqtt transport will not be enabled without proxy configuration\n");
	    fflush(stderr);
	    return(-1);
    }
...
//fflush(stdout);
	...
	pthread_mutex_lock(&MQTT_Proxy.conn_lock);
//printf("create_conn: inside lock 2\n");
//fflush(stdout);
	hv.v = (void *)conn;
	(void)RBInsertI(MQTT_Proxy.connections,conn->sd,hv);
	pthread_mutex_unlock(&MQTT_Proxy.conn_lock);
//printf("create_conn: outside lock 2\n");
//fflush(stdout);
...
int cmq_mqtt_accept(int sd, unsigned long timeout)
{
	...
	pthread_mutex_lock(&MQTT_Proxy.conn_lock);
//printf("accept: inside lock\n");
//fflush(stdout);
	rb = RBFindI(MQTT_Proxy.connections,(int)sd);
	pthread_mutex_unlock(&MQTT_Proxy.conn_lock);
//printf("accept: outside lock\n");
//fflush(stdout);
	if(rb == NULL) {
...
int cmq_mqtt_recv_msg(int sd, unsigned char **fl)
{
	...
	pthread_mutex_lock(&MQTT_Proxy.conn_lock);
//printf("recv: inside lock\n");
//fflush(stdout);
	rb = RBFindI(MQTT_Proxy.connections,sd);
	pthread_mutex_unlock(&MQTT_Proxy.conn_lock);
//printf("recv: outside lock\n");
//fflush(stdout);
	if(rb == NULL) {
