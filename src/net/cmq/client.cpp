	...
	if(has_cap == 1) {
		...
		if((SearchKeychain(cap_file,(char *)std::string(woof_name).c_str(),&cap) >= 0) ||
			...
			if(new_cap != NULL) {
				...
				err = cmq_frame_append(fl,f);
				if(err < 0) {
					DEBUG_WARN("Could not append cap frame for Get with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					cmq_frame_destroy(f);
					return(-1);
				}
...
int32_t backend::remote_get_range(std::string_view woof_name_v, 
		void* elem, uint32_t elem_size, uint32_t seq_no, uint32_t count) {
	std::lock_guard<std::mutex> lock(CMQ_lock);
	std::string woof_name(woof_name_v);
	auto ip = ip_from_woof(woof_name);
	auto port = port_from_woof(woof_name);
	...
	err = cmq_frame_list_create(&fl);
	if(err < 0) {
		DEBUG_WARN("Could not connect create msg for WoofMsgGetRange");
		printf("WooFMsgGetRange: could not create msg\n");
		return -1;
	}
	...
	if(has_cap == 1) {
		...
		if((SearchKeychain(cap_file,(char *)std::string(woof_name).c_str(),&cap) >= 0) ||
			...
			if(new_cap != NULL) {
				// tag first
				t_str = std::to_string(WOOF_MSG_GET_RANGE_CAP).c_str();
				err = cmq_frame_create(&f,(unsigned char *)t_str,strlen(t_str)+1);
				if(err < 0) {
					DEBUG_WARN("Could not create tag frame for GetRange with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					free(new_cap);
					return(-1);
				}
				err = cmq_frame_append(fl,f);
				if(err < 0) {
					DEBUG_WARN("Could not append tag frame for GeRange with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					cmq_frame_destroy(f);
					free(new_cap);
					return(-1);
				...
				free(new_cap);
				if(err < 0) {
					DEBUG_WARN("Could not create cap frame for GetRange with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					return(-1);
				}
				err = cmq_frame_append(fl,f);
				if(err < 0) {
					DEBUG_WARN("Could not append cap frame for GetRange with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					cmq_frame_destroy(f);
					return(-1);
				}
	...
	// tag is in first frame
	if(has_cap == 0) {
		const char *cmd = std::to_string(WOOF_MSG_GET_RANGE).c_str();
		err = cmq_frame_create(&f,(unsigned char *)cmd,strlen(cmd)+1);
		if(err < 0) {
			DEBUG_WARN("Could not connect create cmd for WoofMsgGetRange");
		...
		err = cmq_frame_append(fl,f);
		if(err < 0) {
			DEBUG_WARN("Could not connect append cmd for WoofMsgGetRange");
			printf("WooFMsgGetRange: could not append cmd\n");
			cmq_frame_list_destroy(fl);
			cmq_frame_destroy(f);
			return -1;
		}
	...
	err = cmq_frame_create(&f,(unsigned char *)w_ptr,strlen((const char *)w_ptr)+1);
	if(err < 0) {
        	DEBUG_WARN("Could not connect create woof_name for WoofMsgGetRange");
		printf("WooFMsgGetRange: could not create woof_name\n");
		cmq_frame_list_destroy(fl);
		return -1;
	}
	// add woof name fframe to msg
	err = cmq_frame_append(fl,f);
	if(err < 0) {
        	DEBUG_WARN("Could not connect append woof_name for WoofMsgGetRange");
		printf("WooFMsgGetRange: could not append woof_name\n");
		cmq_frame_list_destroy(fl);
		cmq_frame_destroy(f);
		return -1;
	}
	...
	err = cmq_frame_create(&f,(unsigned char *)sn,strlen(sn)+1);
	if(err < 0) {
        	DEBUG_WARN("Could not connect create seqno for WoofMsgGetRange");
		printf("WooFMsgGet: could not create seq_no\n");
		cmq_frame_list_destroy(fl);
		return -1;
    	}
	...
	err = cmq_frame_append(fl,f);
	if(err < 0) {
		DEBUG_WARN("Could not connect append seq_no for WoofMsgGetRange");
		printf("WooFMsgGet: could not append seq_no\n");
		cmq_frame_list_destroy(fl);
		cmq_frame_destroy(f);
		return -1;
	...
	// convert count to string and create frame
	const char *co = std::to_string(count).c_str();
	err = cmq_frame_create(&f,(unsigned char *)co,strlen(co)+1);
	if(err < 0) {
        	DEBUG_WARN("Could not connect create count for WoofMsgGetRange");
		printf("WooFMsgGet: could not create count\n");
		cmq_frame_list_destroy(fl);
		return -1;
    	}
	// add seq_no frame to request msg
	err = cmq_frame_append(fl,f);
	if(err < 0) {
		DEBUG_WARN("Could not connect append count for WoofMsgGetRange");
		printf("WooFMsgGet: could not append count\n");
		cmq_frame_list_destroy(fl);
		cmq_frame_destroy(f);
		return -1;
	}
...
//fflush(stdout);
	if(sd < 0) {
		DEBUG_WARN("Could not connect to server for WoofMsgGetRange");
		printf("WooFMsgGetRange: server connect failed to %s:%d\n",
			c_ip_str,stoi(port_str));
		cmq_frame_list_destroy(fl);
		return -1;
	}
	...
	if(err < 0) {
		DEBUG_WARN("Could not send to server for WoofMsgGetRange");
		printf("WooFMsgGetRange: server request send failed to %s:%d\n",
			c_ip_str, stoi(port_str));
		cmq_frame_list_destroy(fl);
		cmq_pkt_err_close(sd);
		return -1;
...
#endif
	if(err < 0) {
		DEBUG_WARN("Could not receive reply for WoofMsgGetRange");
		printf("WooFMsgGetRange: server request recv failed from %s:%d\n",
			c_ip_str,stoi(port_str));
	...
	// probably paranoid
	if(cmq_frame_list_empty(r_fl)) {
		DEBUG_WARN("Empty receive reply for WoofMsgGetRange");
		printf("WooFMsgGetRange: empty recv from %s:%d\n",
			c_ip_str,stoi(port_str));
		perror("WooFMsgGetRange");
		cmq_frame_list_destroy(r_fl);
		return -1;
	...
	// pop the response frame
    	err = cmq_frame_pop(r_fl, &r_f);
	if(err < 0) {
		DEBUG_WARN("msg pop failed receive reply for WoofMsgGetRange");
		printf("WooFMsgGetRange: pop failed for recv from %s:%d\n",
			c_ip_str,stoi(port_str));
		perror("WooFMsgGetRange");
		cmq_frame_list_destroy(r_fl);
		return -1;
	...
	if((cmq_frame_payload(r_f) == NULL) ||
		(cmq_frame_size(r_f) == 0))
	{
		DEBUG_WARN("empty frame receive reply for WoofMsgGetRange");
		printf("WooFMsgGetRange: empty frame for recv from %s:%d\n",
			c_ip_str,stoi(port_str));
		//perror("WooFMsgGet");
		cmq_frame_destroy(r_f);
		return -1;
	...
	// copy contents of recv element
	std::memcpy(elem, cmq_frame_payload(r_f), cmq_frame_size(r_f));
	// compute the count of returns elements -> could be short
	r_count = ((int32_t)cmq_frame_size(r_f)/(int32_t)elem_size); 
...
int32_t backend::remote_create(std::string_view woof_name_v, uint32_t elem_size, uint32_t history_size) {
	...
	err = cmq_frame_list_create(&fl);
	if(err < 0) {
		DEBUG_WARN("Could not connect create msg for WoofMsgCreate");
		printf("WooFMsgCreate: could not create msg\n");
		return -1;
	}
	...
	if(has_cap == 1) {
		...
		if((SearchKeychain(cap_file,(char *)std::string(woof_name).c_str(),&cap) >= 0) ||
			...
			if(new_cap != NULL) {
				// tag first
				t_str = std::to_string(WOOF_MSG_CREATE_CAP).c_str();
				err = cmq_frame_create(&f,(unsigned char *)t_str,strlen(t_str)+1);
				if(err < 0) {
					DEBUG_WARN("Could not create tag frame for Create with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					free(new_cap);
					return(-1);
				}
				err = cmq_frame_append(fl,f);
				if(err < 0) {
					DEBUG_WARN("Could not append tag frame for Create with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					cmq_frame_destroy(f);
					free(new_cap);
					return(-1);
				...
				free(new_cap);
				if(err < 0) {
					DEBUG_WARN("Could not create cap frame for Create with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					return(-1);
				}
				err = cmq_frame_append(fl,f);
				if(err < 0) {
					DEBUG_WARN("Could not append cap frame for Create with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					cmq_frame_destroy(f);
					return(-1);
				}
	...
	// Create is only enabled if there is a valud CAP
	if(has_cap == 0) {
		DEBUG_WARN("No valid cap on Create");
		return -1;
	}
	// convert woof name as a string view to char *
	// and create frame with it
	...
	err = cmq_frame_create(&f,(unsigned char *)w_ptr,strlen((const char *)w_ptr)+1);
	if(err < 0) {
        	DEBUG_WARN("Could not connect create woof_name for WooFMsgCreate");
		printf("WooFMsgCreate: could not create woof_name\n");
		cmq_frame_list_destroy(fl);
		return -1;
	}
	// add woof name fframe to msg
	err = cmq_frame_append(fl,f);
	if(err < 0) {
        	DEBUG_WARN("Could not connect append woof_name for WooFMsgCreate");
		printf("WooFMsgCreate: could not append woof_name\n");
		cmq_frame_list_destroy(fl);
		cmq_frame_destroy(f);
		return -1;
	}

	// convert elem_size to string and create frame
	const char *el_str = std::to_string(elem_size).c_str();
	err = cmq_frame_create(&f,(unsigned char *)el_str,strlen(el_str)+1);
	if(err < 0) {
        	DEBUG_WARN("Could not connect create el_size for WooFMsgCreate");
		printf("WooFMsgCreate: could not create el_size\n");
		cmq_frame_list_destroy(fl);
		return -1;
    	}
	// add el_size frame to request msg
	err = cmq_frame_append(fl,f);
	if(err < 0) {
		DEBUG_WARN("Could not connect append el_size for WooFMsgCreate");
		printf("WooFMsgCreate: could not append el_size\n");
		cmq_frame_list_destroy(fl);
		cmq_frame_destroy(f);
		return -1;
	}

	// convert history_size to string and create frame
	const char *hist_str = std::to_string(history_size).c_str();
	err = cmq_frame_create(&f,(unsigned char *)hist_str,strlen(hist_str)+1);
	if(err < 0) {
        	DEBUG_WARN("Could not connect create history_size for WooFMsgCreate");
		printf("WooFMsgCreate: could not create history_size\n");
		cmq_frame_list_destroy(fl);
		return -1;
    	}
	// add history_size frame to request msg
	err = cmq_frame_append(fl,f);
	if(err < 0) {
		DEBUG_WARN("Could not connect append history_size for WooFMsgCreate");
		printf("WooFMsgCreate: could not append history_size\n");
		cmq_frame_list_destroy(fl);
		cmq_frame_destroy(f);
		return -1;
	}
...
//fflush(stdout);
	if(sd < 0) {
		DEBUG_WARN("Could not connect to server for WooFMsgCreate");
		printf("WooFMsgCreate: server connect failed to %s:%d\n",
			c_ip_str,stoi(port_str));
		cmq_frame_list_destroy(fl);
		return -1;
	}
	...
	if(err < 0) {
		DEBUG_WARN("Could not send to server for WooFMsgCreate");
		printf("WooFMsgGet: server request send failed to %s:%d\n",
			c_ip_str, stoi(port_str));
		cmq_frame_list_destroy(fl);
		cmq_pkt_err_close(sd);
	...
	if(err < 0) {
		...
		printf("WooFMsgCreate: server request recv failed from %s:%d\n",
			c_ip_str,stoi(port_str));
		perror("WooFMsgGet");
		cmq_pkt_err_close(sd);
		return -1;
	...
	// probably paranoid
	if(cmq_frame_list_empty(r_fl)) {
		DEBUG_WARN("Empty receive reply for WooFMsgCreate");
		printf("WooFMsgCreate: empty recv from %s:%d\n",
			c_ip_str,stoi(port_str));
	...
	if(err < 0) {
		DEBUG_WARN("msg pop failed receive reply for WoofMsgGet");
		printf("WooFMsgCreate: pop failed for recv from %s:%d\n",
			c_ip_str,stoi(port_str));
		perror("WooFMsgCreate");
		cmq_frame_list_destroy(r_fl);
		return -1;
	...
	// zero frame indicates frame error
	if((cmq_frame_payload(r_f) == NULL) ||
		(cmq_frame_size(r_f) == 0))
	{
		DEBUG_WARN("empty frame receive reply for WooFMsgCreate");
		printf("WooFMsgCreate: empty frame for recv from %s:%d\n",
			c_ip_str,stoi(port_str));
		//perror("WooFMsgGet");
		cmq_frame_destroy(r_f);
		return -1;
...
backend::remote_put(std::string_view woof_name_v, const char* handler_name, const void* elem, uint32_t elem_size) {
	...
	err = cmq_frame_list_create(&fl);
	if(err < 0) {
        	DEBUG_WARN("Could not create message for Put for %s", woof_name.c_str());
		return(-1);
	}
	...
	if(has_cap == 1) {
		...
		if((SearchKeychain(cap_file,(char *)std::string(woof_name).c_str(),&cap) >= 0) ||
			...
			if(new_cap != NULL) {
				...
				err = cmq_frame_append(fl,f);
				if(err < 0) {
					DEBUG_WARN("Could not append cap frame for Put with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					cmq_frame_destroy(f);
					return(-1);
				}
	...
	if(has_cap == 0) {
		t_str = std::to_string(WOOF_MSG_PUT).c_str();
		err = cmq_frame_create(&f,(unsigned char *)t_str,strlen(t_str)+1);
		if(err < 0) {
...
int32_t backend::remote_get_latest_seq_no(std::string_view woof_name_v,
	...
	err = cmq_frame_list_create(&fl);
	if(err < 0) {
        	DEBUG_WARN("Could not create message for GetLatestSeqnowithCAP for %s %s %d", 
				woof_name.c_str(),
				cause_woof_name,
				(int)cause_woof_latest_seq_no);
        	printf("Could not create message for GetLatestSeqnowithCAP for %s %s %d", 
				woof_name.c_str(),
				cause_woof_name,
				(int)cause_woof_latest_seq_no);
		return(-1);
	...
	if(has_cap == 1) {
		...
		if((SearchKeychain(cap_file,(char *)std::string(woof_name).c_str(),&cap) >= 0) ||
			...
			if(new_cap != NULL) {
				// tage first
				t_str = std::to_string(WOOF_MSG_GET_LATEST_SEQNO_CAP).c_str();
				err = cmq_frame_create(&f,(unsigned char *)t_str,strlen(t_str)+1);
				if(err < 0) {
				...
				err = cmq_frame_append(fl,f);
				if(err < 0) {
					DEBUG_WARN("Could not append cap frame for GetLatestSeqno with cap for %s", woof_name.c_str());
					cmq_frame_list_destroy(fl);
					cmq_frame_destroy(f);
					return(-1);
				}
	...
	if(has_cap == 0) {
		// tag is first
		t_str = std::to_string(WOOF_MSG_GET_LATEST_SEQNO).c_str();
		err = cmq_frame_create(&f,(unsigned char *)t_str,strlen(t_str)+1);
		if(err < 0) {
			DEBUG_WARN("Could not create tag frame for GetLatestSeqnowithCAP for %s", woof_name.c_str());
			cmq_frame_list_destroy(fl);
			return(-1);
		}
		...
		err = cmq_frame_append(fl,f);
		if(err < 0) {
			DEBUG_WARN("Could not append tag frame for GetLatestSeqnowithCAP for %s", woof_name.c_str());
			cmq_frame_list_destroy(fl);
			cmq_frame_destroy(f);
			return(-1);
		}
