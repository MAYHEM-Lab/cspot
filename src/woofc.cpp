 ...
 * of #name# is a fully qualified URI or a local file name.  Remote create is only
 * supported using a capability (enforced in WooFMsgCreate())
 */
int WooFCreate(const char* name, unsigned long element_size, unsigned long history_size) 
{
	...
	* function cancel if we wrap (stored in ELID)
	*/
...
/*
 * sets the starting seq_no to #new_seqno#
 *
 * used to start replacements woofs at the old seq_no
 */
...
WOOF* WooFOpen(const char* name) 
{
	...
	if (wf == NULL) {
		fprintf(stderr, "WooFOpen: no space for woof struct, %s failed\n", local_name);
		MIOClose(mio);
		return (NULL);
	}
...
unsigned long WooFAppend(WOOF* wf, const char* hand_name, const void* element) 
{
	...
	*/
	...
	if ((ls = LogEvent(Name_log, ev.get())) == 0) {
		fprintf(stderr,"WooFAppend: couldn't log event to log\n");
		return -1;
	}
...
unsigned long WooFPut(const char* wf_name, const char* wf_handler, const void* element) 
{
	...
	if (!wf) {
		fprintf(stderr,"WooFPut: could not open woof %s\n",wf_name);
		return (-1);
	}
                               ...
                               unsigned long long cause_seq_no) 
{
	...
	*/
	...
	if (wf == NULL) {
		fprintf(stderr,"WooFPutWithCause: couldn't open %s\n", wf_name);
		return (-1);
	}
...
int WooFGet(const char* wf_name, void* element, unsigned long seq_no) 
{
	...
	if (wf == NULL) {
		fprintf(stderr,"WooFGet: could not open %s\n",wf_name);
		return (-1);
	}
	...
	void* elements, unsigned long seq_no, unsigned int count) 
{
	...
	if((int)count < 0) {
		fprintf(stderr,"WooFGetRange: bad count %d\n",count);
		return(-1);
	}
	if(count == 0) {
		return(0);
	...
	if (IsRemoteWoof(wf_name)) {
		...
		el_size = WooFGetElSize(NULL,wf_name);
		if(el_size == (unsigned long) -1) {
			fprintf(stderr,"WooFGetRange: could not get element size for %s\n",
					wf_name);
			return(-1);
		}
	}
	...
	if (wf == NULL) {
		fprintf(stderr,"WooFGetRange: could not open %s\n",wf_name);
		return (-1);
	}
                                          ...
                                          unsigned long cause_woof_latest_seq_no) 
{
	...
	wf = WooFOpen(wf_name);
	if (wf == NULL) {
		fprintf(stderr,"WooFGetLatestSeqno: could not get seqno from %s\n",wf_name);
		return (-1);
	}
...
unsigned long WooFGetEarliestSeqno(const char* wf_name)
{
	...
	wf = WooFOpen(wf_name);
	if (wf == NULL) {
		fprintf(stderr,"WooGetEarliestSeqno: could not open %s\n",wf_name);
		return (-1);
	}
