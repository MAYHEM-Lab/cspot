...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < size; i++) {
        ...
        err = GLogEvent(glog, ev);
        if (err < 0) {
            fprintf(stderr, "glogevent %d failed\n", i);
            exit(1);
        }
    ...
    if (err < 0) {
        fprintf(stderr, "error logging second function arrival at host 2\n");
        exit(1);
    }
    GLogPrint(stdout, glog_2);
    ...
    if (err < 0) {
        fprintf(stderr, "trouble importing host 2 local log to host 1\n");
        fflush(stderr);
        exit(1);
    }
    ...
    err = ImportLogTail(glog_2, llog);
    if (err < 0) {
        fprintf(stderr, "trouble importing host 1 local log to host 2\n");
        fflush(stderr);
        exit(1);
    }
    ...
    if (err < 0) {
        fprintf(stderr, "trouble importing host 2 local log to host 3\n");
        fflush(stderr);
        exit(1);
    }
    ...
    if (err < 0) {
        fprintf(stderr, "trouble importing host 3 local log to host 2\n");
        fflush(stderr);
        exit(1);
    }
