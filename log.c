...
void LogPrint(FILE* fd, LOG* log) {
    ...
    while (curr != log->tail) {
        fprintf(fd,
                "\t[%lu] host: %lu seq_no: %llu r_host: %lu r_seq_no: %llu woofc_seq_no: "
                "%lu woof: %s type: %d timestamp: %lu\n",
                curr,
                ev_array[curr].host,
...
int LogResolveHorizon(GLOG* gl, EVENT* event) {
    ...
    this_event = PendingFindEvent(gl->pending, event->host, event->seq_no);
    ...
    if (this_event->cause_seq_no < host->event_horizon) {
         ...
         * list
         */
        new_ev = EventCreate(this_event->type, this_event->host);
        if (new_ev == NULL) {
...
void GLogPrintPending(GLOG* gl) {
    ...
    RB_FORWARD(gl->pending->alive, rb) {
        ...
        ev = (EVENT*)rb->value.v;
        printf("glog %lu: psat %lu %llu (max: %llu eh: %llu) %lu %llu (max: %llu eh: "
               "%llu)\n",
               gl->host_id,
               ev->host,
               ev->seq_no,
...
#ifdef DEBUG
            printf("pendsatisfied %lu: ev with cause %lu %llu cause %lu %llu cause max: "
                   "%llu\n",
                   gl->host_id,
                   ev->host,
                   ev->seq_no,
...
int IsAnchor(EVENT* ev) {
    if ((ev->host == ev->cause_host) && (ev->seq_no == ev->cause_seq_no)) {
        return (1);
    } else {
        return (0);
    }
...
int GLogEvent(GLOG* gl, EVENT* event) {
     ...
     */
    cause_event = PendingFindEvent(gl->pending, event->cause_host, event->cause_seq_no);
    ...
    while (this_event != NULL) {
        ...
        if (cause_host == NULL) {
            err = HostListAdd(gl->host_list, event->cause_host);
            if (err < 0) {
                fprintf(stderr, "couldn't add cause host %lu\n", this_event->cause_host);
                fflush(stderr);
                return (-1);
            }
            cause_host = HostListFind(gl->host_list, this_event->cause_host);
            ...
            this_event = PendingFindCause(gl->pending, host_id, seq_no);
            continue;
        }
        /*
...
int ImportLogTail(GLOG* gl, LOG* ll) {
    ...
    if (host != NULL) {
        ...
        if (err < 0) {
            fprintf(stderr, "couldn't fast forward from host %lu\n", remote_host);
            fflush(stderr);
            LogFree(lt);
            return (-1);
        }
