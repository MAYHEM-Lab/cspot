...
Currently, CSPOT builds from source using a bash installation script on Ubuntu
20.04 or later.  CentOS stream, Rocky, and Alma have been supported in the
past, and can be made to work, but the source repository is tested using
Ubuntu.

CSPOT was originally written to install on cloud instances.  Thus, the
installation scripts (written in bash) run as root and are not "gentle" with
respect to updating and installing any needed dependencies.  If you are
installing CSPOT on a server, please read the installation scripts to make
sure there are no conflicts with respect to the dependencies.

CSPOT can also run in two modes: containerized (using docker) and Linux process 
isolated.  The default mode uses Linux processes.  If you want to use docker
for further isolation, then use the install-ubuntu-docker.sh script.
Otherwise, use install-ubuntu-nodocker.sh.  The docker install script will
install docker ce which requires removing the docker that ships with older
version of ubuntu.  If you are already using docker, then use the no-docker
version to install and the script will not disturb your existing installation.

Note that CSPOT uses static compilation of C and C++ which has been removed from Ubuntu
24.04.  Thus, The following shows the installation procedure for Ubuntu 20.04.
However, once compiled and linked, the binaries can be copied to new versions
of Ubuntu.
