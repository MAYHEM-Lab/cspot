...
and the installer will put the binary distribution in the cspot subdirectory.
Not that the installer will install the install script as well so future
updates need only run
[source sh]
----
cd cspot
./update-cspot-distributions.sh
----
to get the latest.  Note that this script uses CSPOT capabilities (see below)
...
read-only capability will be able to read (call Get) on any WOOF in the
namespace that is not further protected by a capability.  Any process holding
the read-write capability will be able to either read (Get) or write (Put) but
...
CSPOT.CAP that contains the principle capability.
