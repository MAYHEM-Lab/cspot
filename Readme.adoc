...
Reference: R. Wolski,
C. Krintz, F. Bakir, G. George, and W-T. Lin, CSPOT: Portable, Multi-scale
Functions-as-a-Service for IoT, ACM Symposium on Edge Computing (SEC), Nov 2019,
https://sites.cs.ucsb.edu/~ckrintz/papers/cspot19.pdf
...
CSPOT binaries, run the following command.

[source sh]
----
mkdir -p cspot
cd cspot
rm -rf update-cspot-distribution.sh
(curl -fsSL https://raw.githubusercontent.com/MAYHEM-Lab/cspot/release-3.0/dist/update-cspot-distribution.sh | /bin/bash)
----
...
----
cd cspot
./update-cspot-distribution.sh release
----
...
[source,sh]
----
pkill -x woofc-container
----

This command will terminate the `woofc-container` process, which will also tear
