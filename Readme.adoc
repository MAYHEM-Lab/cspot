...
the data that has yet to be forwaded in order with the original timestamps so
...
WOOFs that are suffciently large enough to hold them.  
...
ommitted, the command fetches the latest version.
...
the namep[sace on a port between 50000 and 60000.  As a result, the server
