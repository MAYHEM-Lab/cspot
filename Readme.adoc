...
capabilitie's holder and CSPOT uses the least privileged capability to sign a
CSPOT message.  Currently, all messages require at least READ privilige, the
...
have all group and worl permissions disabled.
...
"highest" permission need be set to convery all lower permissions.  Thus, "-p
...
capability for a specific WOOF or WOOFs, then it will check accress rights on API calls
for those WOOFs.
...
the indentation when it determmines whether the capability that is present in the
file is valid.
...
API call will succeed.  If not, it will fail because woofc-namepsace-platform
...
capabilties.yaml keychain file).  Instead, it signs the message payload with
the capability (possibly annnuating it down to the minimum access level) 
on the client side and the server derives a capabilty (from the principle)
...
capabilities and use CAPlets attenuations to implement distributed access
control.
